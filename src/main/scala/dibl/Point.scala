...
case class Point(ringRadius: Double, dotNr: Int, ringNr: Int, s: Settings) {
  ...
  val x: Double = ringRadius * Math.cos(a) + s.outerRadius + s.maxArc / 2

  val y: Double = ringRadius * Math.sin(a) + s.outerRadius + s.maxArc / 2

  /** distance between two dots along the circle */
  val arcLength: Double = Math.PI * ringRadius / s.dotsPerRing
